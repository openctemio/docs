...
The goal is to complete the **Source Code Security Scanning** feature to:
1. Scan source code (SAST, SCA, Secrets, Container)
2. Embed into CI/CD GitLab as separate steps
3. Track finding lifecycle with activity log
4. Display full context (branch, commit, PR, author)
5. Component/dependency management with vulnerability tracking
6. Developer view per tenant

---

## Existing System Analysis

### Finding Comments (Already exists - Migration 007)

**Table**: `finding_comments`
- **Purpose**: User comments + Status change records
- **Features**:
...
- Handler: Endpoints in `vulnerability_handler.go`

### Relationship with Finding Activities (New)
...
- When `finding_comment` is created → auto-create `finding_activity` type=`comment_added`
- When `finding_comment` is updated → auto-create `finding_activity` type=`comment_updated`
- When `finding_comment` is deleted → auto-create `finding_activity` type=`comment_deleted`
- Status change comments already have `is_status_change=true` → also create a separate activity
...
> **Note**: OpenCTEM already has an Agent CLI (`agent`) with scanning capabilities. This section describes the necessary enhancements, NOT creating a new CLI.
...
### 4.2 Existing Commands (Already available)
...
The AI Triage feature enables automatic analysis and assessment of Findings using AI:
- Analyze the context of a finding (code snippet, location, severity)
- Suggest a status (Valid, False Positive, Duplicate, etc.)
